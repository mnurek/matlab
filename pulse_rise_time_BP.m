function R = pulse_rise_time_BP( t, y, thr_low, thr_high, interp_N, reference_levels )
%PULSE_RISE_TIME Calculate rise time of the pulse
%
%   Calculates rise time of the provided pulse. The method uses MATLAB function 'risetime'. By default, the method
%   returns 10% to 90% rise time. Alternatively, one can provide low and high thresholds. Finally, it is possible to use
%   FFT interpolation to increase accuracy of rise time estimation in case of low sampling densities. If the method is
%   called with single argument, then the time vector is assumed to be equal to the sample index.
%
%   R = pulse_rise_time( y )
%   R = pulse_rise_time( t, y )
%   R = pulse_rise_time( t, y, thr_low, thr_high )
%   R = pulse_rise_time( t, y, thr_low, thr_high, interp_N )
%   R = pulse_rise_time( t, y, thr_low, thr_high, interp_N, reference_levels )
%
%   Parameters:
%       t - time vector
...
%
%   Returns:
%       R - measured rise time of the pulse
...
% Calculate rise time (ensure that we only measure one edge). If more edges are found then increase state levels
...
while length(R) < 1 && ref_levels_tolerance < percent_of_levels(1)
    R = risetime(y, t, 'PercentReferenceLevels', percent_of_levels, 'StateLevels', reference_levels, 'Tolerance', ref_levels_tolerance);
    ...
    if length(R) < 1
        warning('Could not estimate rise time, increasing state levels tolerance to %.1f %%', ref_levels_tolerance);
    end
